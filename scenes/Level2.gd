...
var next_level
var next_level_path = "res://scenes/Level3.tscn"
var border_points
var next_points
var moving_points
	...
	if Input.is_action_just_pressed("ui_accept"):
		for point in border_points:
			if next_points.has(point.get_name()):
				...
				if (get_point_position(index).distance_to(next_level.get_point_position(index)) != 0):
					moving_points[index] = true
					print(get_point_position(index).distance_to(next_level.get_point_position(index + 2)))
